...
void LevelsState::handleEvent(const sf::Event& event){
    if(const auto* textEntered=event.getIf<sf::Event::TextEntered>()){
        ...
        if((unicode>'0' && unicode<='9' && input.size()<2) || (unicode=='0' && input.size()>0)){   //qua accettiamo i livelli da 1 a 10
            ...
            if(std::stoul(input)>utils::lastLvl){
                input="10";
            }
    ...
    if(const auto* keyPressed=event.getIf<sf::Event::KeyPressed>()){
        if(keyPressed->code==sf::Keyboard::Key::Enter){
            if(input.size()>0  && static_cast<unsigned>(std::stoul(input))<=userLevel){
                game.changeState(std::make_unique<PlayingState>(game, std::stoul(input), 0));   //stoul string to unsigned
            }
        }
